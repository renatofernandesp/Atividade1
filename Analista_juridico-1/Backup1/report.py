...
def gerar_pdf(total, nivel_map, dados_area, pares, analises, niveis_dict) -> bytes:
    buffer = io.BytesIO()
    ...
    header_data = [[Paragraph("⚖️  Relatório — Analisador Jurídico IA", s["titulo"])],
                   [Paragraph(f"Gerado em {agora}", s["subtitulo"])]]
    ...
    if pares:
        ...
        r1_scores, rl_scores, cob_scores = [], [], []
        for texto, justificativa, base_legal in pares:
            sc = scorer.score(texto, justificativa)
        ...
        cob = sum(cob_scores) / len(cob_scores)

        story.append(Paragraph("Métricas de Qualidade da IA", s["secao"]))
        met_rows = [
            ["Métrica", "Score", "Descrição"],
            ["ROUGE-1", f"{r1:.3f}", "Sobreposição de palavras entre texto e justificativa"],
            ["ROUGE-L", f"{rl:.3f}", "Maior subsequência comum entre texto e justificativa"],
