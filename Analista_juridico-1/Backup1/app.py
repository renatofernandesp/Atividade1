...
from analyzer import analisar_lote, NIVEIS, AREAS, MODELOS
        ...
        for idx, modelo_ativo in enumerate(modelos_keys):
            with abas[idx]:
                ...
                <div style='background:#f8f9fa;border-radius:10px;padding:20px 24px;margin-bottom:8px'>
                    ...
                    <b>ROUGE-1</b> e <b>ROUGE-L</b> medem sobreposição entre o texto original e a justificativa gerada.
                ...
                if not pares:
                    ...
                    r1_scores, rl_scores, cobertura_scores = [], [], []

                    for texto, justificativa, base_legal in pares:
                        s = scorer.score(texto, justificativa)
                    ...
                    r1_med  = sum(r1_scores)  / len(r1_scores) if r1_scores else 0
                    rl_med  = sum(rl_scores)  / len(rl_scores) if rl_scores else 0
                    ...
                    with m1:
                        st.markdown(f"<p style='text-align:center;font-weight:600;margin-bottom:0'>ROUGE-1</p>", unsafe_allow_html=True)
                        st.plotly_chart(gauge(r1_med, "ROUGE-1", "#0066cc"), use_container_width=True, key=f"g1_{idx}")
                    with m2:
                        st.markdown(f"<p style='text-align:center;font-weight:600;margin-bottom:0'>ROUGE-L</p>", unsafe_allow_html=True)
                        st.plotly_chart(gauge(rl_med, "ROUGE-L", "#33bbff"), use_container_width=True, key=f"g2_{idx}")
                    with m3:
